...
class HeaderSetter {
  ...
  'content-type'(val) {
    ...
    if (!mime.contentType(val)) {
      throw new TypeError('Invalid content type: ' + val)
    }
    this.res.setHeader('Content-Type', mime.contentType(val))
  }
  'string'(field, val) {
  ...
  'object'(field) {
    Object.keys(field).forEach(key => {
      this[typeof field[key]](key, field[key])
    })
  }
  'undefined'(field, val) {
    throw new TypeError('header field cannot be undefined')
  }
}
...
class ExpressResponse extends http.ServerResponse {
  ...
  sendFile(path, options, callback) {
    ...
    // transfer
    this.#sendfile(res, file, opts, function (err) {
      if(err) {
  ...
  header(field, val) {
    const setter = new HeaderSetter(this)
    setter[typeof field](field, val)
    return this
  }
